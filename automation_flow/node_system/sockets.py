...
def _translated_indexed_socket_label(text):
    ...
    for prefix in (
        ...
        if suffix.isdigit():
            return f"{af_iface(prefix.strip())} {suffix}"
    return af_iface(raw_text) if raw_text else raw_text
...
class AFBaseInterfaceSocket(bpy.types.NodeTreeInterfaceSocket):
    ...
    def draw(self, context, layout):
        del context
        display_text = af_iface(self.name if self.name else self.bl_label)
...
class AFSocketPropertyDefinition(AFBaseSocket):
    ...
    def draw(self, context, layout, node, text):
        del context, node
        layout.label(text=af_iface(text if text else self.bl_label), translate=False)
