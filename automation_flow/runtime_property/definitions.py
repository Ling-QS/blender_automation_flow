...
def _property_assignment_has_content(property_assignment, node_name):
    return len(_iter_property_assignment_entries(property_assignment, node_name)) > 0
...
def _property_definition_matches_any_filter(target_definition, filter_definition, node_name):
    target_definition = _validate_property_definition(target_definition, node_name)
    filter_definition = _validate_property_definition(filter_definition, node_name)
    filter_entries = _iter_property_definition_entries(filter_definition, node_name)
    if _is_composite_property_definition(target_definition):
        target_entries = _iter_property_definition_entries(target_definition, node_name)
        return any(
            _property_definition_matches_filter(target_entry, filter_entry, node_name)
            for target_entry in target_entries
            for filter_entry in filter_entries
        )
    return any(
        _property_definition_matches_filter(target_definition, filter_entry, node_name)
        for filter_entry in filter_entries
    )
