...
def build_task_node_helpers(
    *,
    ...
    def _auto_flow_bake_target_start_items(self, context):
        del context
        ...
        if node_tree is not None:
            for node in getattr(node_tree, "nodes", []):
                if getattr(node, "bl_idname", "") != "AFNodeStart":
                    continue
                items.append((str(node.name), str(node.name), ""))
        if not items:
            items.append(("__NONE__", iface_("No Start Nodes"), ""))
        return items

    def _new_auto_flow_bake_asset_id():
    ...
    def _sync_evaluate_task_dependencies_sockets(node):
        ...
        if len(getattr(node, "outputs", [])) >= 2:
            if getattr(node.outputs[0], "bl_idname", "") == "AFSocketObjectList":
                node.outputs[0].name = "Dependency Objects"
    ...
    return {
        ...
        "_auto_flow_bake_target_start_items": _auto_flow_bake_target_start_items,
        "_new_auto_flow_bake_asset_id": _new_auto_flow_bake_asset_id,
