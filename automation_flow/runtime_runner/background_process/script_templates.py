...
def background_render_script_text():
    return (
        "import bpy, json, sys, traceback\n"
        "def _bool_arg(value):\n"
        "    return str(value).strip().lower() in {'1', 'true', 'yes', 'on'}\n"
        "args = sys.argv[sys.argv.index('--') + 1:] if '--' in sys.argv else []\n"
        "status_path, scene_name, render_mode, frame, frame_start, frame_end, write_still, use_viewport = args[:8]\n"
        "payload = {}\n"
        "try:\n"
        ...
        "        raise RuntimeError(f\"Scene '{scene_name}' has no active camera\")\n"
        "    if render_mode == 'STILL':\n"
        "        frame_value = int(frame)\n"
        "        scene.frame_set(frame_value)\n"
        "        bpy.ops.render.render(scene=scene.name, write_still=_bool_arg(write_still), use_viewport=_bool_arg(use_viewport))\n"
        "        payload = {'success': True, 'report': {'scene': scene.name, 'mode': 'STILL', 'frame': frame_value, 'camera': scene.camera.name}}\n"
        "    else:\n"
        "        start_value = int(frame_start)\n"
        "        end_value = int(frame_end)\n"
        "        scene.frame_start = start_value\n"
        "        scene.frame_end = end_value\n"
        "        scene.frame_set(start_value)\n"
        "        bpy.ops.render.render(scene=scene.name, animation=True, use_viewport=_bool_arg(use_viewport))\n"
        "        payload = {'success': True, 'report': {'scene': scene.name, 'mode': 'ANIMATION', 'frame_start': start_value, 'frame_end': end_value, 'camera': scene.camera.name}}\n"
        "except Exception as exc:\n"
        "    payload = {'success': False, 'error_code': 'AF_E005', 'error_message': str(exc), 'traceback': traceback.format_exc()}\n"
...
def _auto_flow_bake_reports(runtime_constants, runner):
    if runtime_constants is None or runner is None:
        return []
    auto_flow_bake_kind = getattr(runtime_constants, "TASK_KIND_AUTO_FLOW_BAKE", "AUTO_FLOW_BAKE")
    reports = []
    for task_handle in getattr(runner, "tasks", {}).values():
        if not isinstance(task_handle, dict):
            continue
        if str(task_handle.get("task_kind", "")) != auto_flow_bake_kind:
            continue
        report = dict(task_handle.get("report", {}) or {})
        if not report:
            continue
        report.setdefault("task_kind", auto_flow_bake_kind)
...
try:
    ...
    while True:
        ...
        try:
            ...
            pass
        auto_flow_reports = _auto_flow_bake_reports(runtime_constants, runner)
        if auto_flow_reports:
            final_report = dict(final_report or {})
            final_report["auto_flow_bake_reports"] = auto_flow_reports
...
def background_task_step_script_text():
    loader_block = _automation_flow_loader_function_text(
        [
            ("runtime_runner", "runtime_runner"),
            ("runtime_constants", "runtime_core.constants"),
            ("runtime_bake", "runtime_bake"),
        ]
    )
    template = """
import bpy
...
args = sys.argv[sys.argv.index("--") + 1:] if "--" in sys.argv else []
status_path, package_dir, step_ref_json = args[:3]
payload = {}
try:
    _module, runtime_runner, runtime_constants, runtime_bake = _load_automation_flow(package_dir)
    step_ref = json.loads(step_ref_json)
    tree_name = str(step_ref.get("tree_name", "") or "")
    node_name = str(step_ref.get("node_name", "") or "")
    node_tree = bpy.data.node_groups.get(tree_name)
    if node_tree is None or getattr(node_tree, "bl_idname", "") != "AFNodeTreeType":
        raise RuntimeError(f"Task step tree '{tree_name}' not found")
    step_node = node_tree.nodes.get(node_name)
    if step_node is None:
        raise RuntimeError(f"Task step '{node_name}' not found")
    scene = bpy.context.scene or (bpy.data.scenes[0] if bpy.data.scenes else None)
    if scene is None:
        raise RuntimeError("No scene available for background task execution")
    runner = runtime_runner.FlowRunner(node_tree, scene)
    runner.current_group_path = list(step_ref.get("group_path", []))
    task_ref = runner._get_linked_output(step_node, "Task Ref", "task_ref")
    if task_ref is None:
        raise RuntimeError(f"Task Ref not found for step '{node_name}'")
    task_kind = str(task_ref.get("task_kind", runtime_constants.TASK_KIND_GEOMETRY))
    if task_kind == runtime_constants.TASK_KIND_GEOMETRY:
        runtime_bake._ensure_background_geometry_task_supported(task_ref, step_node.name)
    task_handle = runner._run_task_ref_now(step_node, task_ref)
    report = dict(task_handle.get("report") or {})
    report.setdefault("task_kind", task_kind)
    report["external_process"] = True
    payload = {"success": True, "report": report, "skipped": bool(task_handle.get("skipped", False) or report.get("skipped", False))}
except Exception as exc:
    payload = {
        "success": False,
        "error_code": str(getattr(exc, "code", "AF_E005")),
...
def background_auto_flow_bake_script_text():
    loader_block = _automation_flow_loader_function_text([("runtime_runner", "runtime_runner")])
    template = """
import bpy
import importlib
import importlib.util
...
args = sys.argv[sys.argv.index("--") + 1:] if "--" in sys.argv else []
status_path, package_dir, task_ref_json = args[:3]
payload = {}
try:
    _module, runtime_runner = _load_automation_flow(package_dir)
    task_ref = json.loads(task_ref_json)
    tree_name = str(task_ref.get("source_tree_name", "") or task_ref.get("start_tree_name", "") or "")
    node_name = str(task_ref.get("source_node", "") or "")
    node_tree = bpy.data.node_groups.get(tree_name)
    if node_tree is None or getattr(node_tree, "bl_idname", "") != "AFNodeTreeType":
        raise RuntimeError(f"Auto Flow Bake tree '{tree_name}' not found")
    owner_node = node_tree.nodes.get(node_name)
    if owner_node is None:
        raise RuntimeError(f"Auto Flow Bake node '{node_name}' not found")
    scene = bpy.context.scene or (bpy.data.scenes[0] if bpy.data.scenes else None)
    if scene is None:
        raise RuntimeError("No scene available for background Auto Flow Bake execution")
    runner = runtime_runner.FlowRunner(node_tree, scene)
    result = runner._invoke_auto_flow_bake_task(owner_node, task_ref)
    save_result = bpy.ops.wm.save_mainfile(filepath=str(getattr(bpy.data, "filepath", "") or ""), check_existing=False, compress=False)
    if "FINISHED" not in {str(token) for token in save_result}:
        raise RuntimeError("Failed to save background Auto Flow Bake result blend")
    report = dict(result or {})
    report["external_process"] = True
    report["result_blend_path"] = str(getattr(bpy.data, "filepath", "") or "")
    payload = {"success": True, "report": report, "skipped": bool(report.get("skipped", False))}
except Exception as exc:
    payload = {
        "success": False,
        "error_code": str(getattr(exc, "code", "AF_E005")),
