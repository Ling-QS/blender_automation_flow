...
class RuntimeTaskRefStatusMixin:
    def _geometry_bake_has_existing_cache(self, task_ref, scene, bake_entry):
        return _geometry_bake_has_existing_cache_impl(task_ref, scene, bake_entry)

    def _geometry_bake_has_existing_cache_for_current_entry(self, task_ref, scene, bake_entry):
        return _geometry_bake_has_existing_cache_for_current_entry_impl(task_ref, scene, bake_entry)
    ...
    def _embed_task_ref_status_payload(self, node, task_ref):
        ...
        if task_kind == TASK_KIND_AUTO_FLOW_BAKE and bool(task_ref.get("prediction_skipped", False)):
        ...
        if task_kind == TASK_KIND_GEOMETRY:
            ...
            if not bool(task_ref.get("free_before_bake", False)) and (
                self._geometry_bake_has_existing_cache_for_current_entry(task_ref, self.scene, bake_entry)
                or self._geometry_bake_has_existing_cache(task_ref, self.scene, bake_entry)
            ):
    ...
    def _task_kind_for_task_ref_node(self, node):
        ...
        if node_type == "AFNodeAutoFlowBakeTarget":
            return TASK_KIND_AUTO_FLOW_BAKE
        return ""
