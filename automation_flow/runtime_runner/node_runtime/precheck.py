...
class RuntimeNodePrecheckMixin:
    def _precheck_node(self, node):
        ...
        if node_type == "AFNodeBakeTask":
            ...
            if bool(node.apply_settings_on_run) and node.use_custom_path and not str(node.directory).strip():
            ...
            if _find_single_from_input_socket(node.inputs["Base Prop Assign"])[0] is None and _find_single_from_input_socket(node.inputs["Add Prop Assign"])[0] is None:
            ...
            if _find_single_from_input_socket(node.inputs["Object List"])[0] is None:
                issues.append(_make_issue("AF_E011", "Object List input is not linked", node.name))
        elif node_type == "AFNodeAutoFlowBakeTarget":
            linked_start_ref = _find_single_from_input_socket(node.inputs["Start Ref"])[0]
            target_start_name = str(getattr(node, "target_start_name", "") or "").strip()
            if linked_start_ref is None and (not target_start_name or target_start_name == "__NONE__"):
                issues.append(_make_issue("AF_E011", "Start Ref input is not linked and no Start is selected", node.name))
