...
from ...runtime_core.constants import FlowExecutionError, TASK_KIND_AUTO_FLOW_BAKE
...
class RuntimeLinearFlowMixin:
    ...
    def _collect_auto_flow_read_geometry_dependency_objects(self, task_ref):
        if not isinstance(task_ref, dict):
            return []
        if str(task_ref.get("task_kind", "") or "") != TASK_KIND_AUTO_FLOW_BAKE:
            return []
