...
from ...runtime_core.constants import (
    AUTO_FLOW_BAKE_ACTION_PROP_BAKE_ASSET_ID,
    AUTO_FLOW_BAKE_ACTION_PROP_FRAME_END,
    AUTO_FLOW_BAKE_ACTION_PROP_FRAME_START,
    AUTO_FLOW_BAKE_ACTION_PROP_NODE_NAME,
    AUTO_FLOW_BAKE_ACTION_PROP_START_NODE_NAME,
    AUTO_FLOW_BAKE_ACTION_PROP_START_TREE_NAME,
    AUTO_FLOW_BAKE_ACTION_PROP_TASK_KIND,
    AUTO_FLOW_BAKE_ACTION_PROP_TREE_NAME,
    FlowExecutionError,
...
from ...runtime_state.cache import (
    _auto_flow_bake_action_name_from_task_ref,
    _auto_flow_bake_slot_display_name,
    _find_auto_flow_bake_action_by_identifier,
)


class RuntimeAutoFlowBakeRecordMixin:
    ...
    def _capture_auto_flow_bake_tree_users(self, task_ref):
        records = []
        seen = set()
        for tree_name in (
            str(task_ref.get("source_tree_name", "") or ""),
            str(task_ref.get("start_tree_name", "") or ""),
        ):
            node_tree = bpy.data.node_groups.get(tree_name)
            for candidate_tree in self._collect_af_tree_hierarchy(node_tree):
                candidate_name = str(getattr(candidate_tree, "name", "") or "")
                if not candidate_name or candidate_name in seen:
                    continue
                seen.add(candidate_name)
                records.append((candidate_tree, bool(getattr(candidate_tree, "use_fake_user", False))))
                candidate_tree.use_fake_user = True
        return records

    def _auto_flow_bake_record_obj_item(self, obj):
    ...
    def _find_auto_flow_bake_action(self, bake_asset_id, action_name):
        return _find_auto_flow_bake_action_by_identifier(bake_asset_id, action_name)

    def _clear_auto_flow_bake_action_contents(self, action):
        if action is None:
            return
        try:
    ...
    def _ensure_auto_flow_bake_action_layer(self, action):
        if action is None:
            return None
        layer = action.layers[0] if len(action.layers) > 0 else action.layers.new("Auto Flow Bake")
        if len(layer.strips) == 0:
            layer.strips.new(type="KEYFRAME")
        return layer

    def _tag_auto_flow_bake_action(self, action, task_ref):
        if action is None:
            return
        action.name = str(task_ref.get("action_name", action.name) or action.name)
        action[AUTO_FLOW_BAKE_ACTION_PROP_TASK_KIND] = TASK_KIND_AUTO_FLOW_BAKE
        action[AUTO_FLOW_BAKE_ACTION_PROP_BAKE_ASSET_ID] = str(task_ref.get("bake_asset_id", "") or "")
        action[AUTO_FLOW_BAKE_ACTION_PROP_TREE_NAME] = str(task_ref.get("source_tree_name", "") or "")
        action[AUTO_FLOW_BAKE_ACTION_PROP_NODE_NAME] = str(task_ref.get("source_node", "") or "")
        action[AUTO_FLOW_BAKE_ACTION_PROP_START_TREE_NAME] = str(task_ref.get("start_tree_name", "") or "")
        action[AUTO_FLOW_BAKE_ACTION_PROP_START_NODE_NAME] = str(task_ref.get("start_node_name", "") or "")
        action[AUTO_FLOW_BAKE_ACTION_PROP_FRAME_START] = int(task_ref.get("frame_start", 1) or 1)
        action[AUTO_FLOW_BAKE_ACTION_PROP_FRAME_END] = int(task_ref.get("frame_end", action.get(AUTO_FLOW_BAKE_ACTION_PROP_FRAME_START, 1)) or action.get(AUTO_FLOW_BAKE_ACTION_PROP_FRAME_START, 1))

    def _ensure_auto_flow_bake_action(self, task_ref, clear_existing=False):
        action_name = _auto_flow_bake_action_name_from_task_ref(task_ref)
        bake_asset_id = str(task_ref.get("bake_asset_id", "") or "").strip()
        action = self._find_auto_flow_bake_action(bake_asset_id, action_name)
        if action is None:
            action = bpy.data.actions.new(action_name)
        elif clear_existing:
            self._clear_auto_flow_bake_action_contents(action)
        self._tag_auto_flow_bake_action(action, {**dict(task_ref), "action_name": action_name})
        self._ensure_auto_flow_bake_action_layer(action)
        return action

    def _ensure_auto_flow_bake_action_slot(self, action, obj):
        if action is None or obj is None:
            return None
        expected_identifier = f"OB{_auto_flow_bake_slot_display_name(obj)}"
        for slot in action.slots:
            if str(getattr(slot, "identifier", "") or "") == expected_identifier:
                return slot
        return action.slots.new("OBJECT", _auto_flow_bake_slot_display_name(obj))

    def _ensure_auto_flow_bake_binding(self, obj, action):
        if obj is None or action is None:
            return None
        self._ensure_auto_flow_bake_action_layer(action)
        anim_data = obj.animation_data_create()
        slot = self._ensure_auto_flow_bake_action_slot(action, obj)
    ...
    def _rotation_data_path_for_payload(self, rotation_payload):
        mode = str((rotation_payload or {}).get("mode", "XYZ") or "XYZ")
        if mode == "QUATERNION":
    ...
    def _record_property_package_current_frame(self, node, property_package, frame):
        bake_context = self.auto_flow_bake_context if isinstance(self.auto_flow_bake_context, dict) else None
            ...
            if scope_kind == PROPERTY_PACKAGE_SCOPE_OBJECT:
                ...
                for item in items:
                    ...
                    if shared_action is not None:
                        self._ensure_auto_flow_bake_binding(obj, shared_action)
                    touched_ids.add(int(item["object_id"]))
                    if bake_context is not None:
                        bake_context.setdefault("touched_object_items", {})[int(item["object_id"])] = self._auto_flow_bake_record_obj_item(obj)
                    ...
                    if definition_kind == PROPERTY_DEFINITION_KIND_OBJECT_DISPLAY:
                        ...
                        if "rotation" in selected_properties and "rotation" in props:
                            ...
                            if self._keyframe_insert_safe(obj, self._rotation_data_path_for_payload(props["rotation"]), frame, node):
                                key_insert_count += 1
...
__all__ = ["RuntimeAutoFlowBakeRecordMixin"]
