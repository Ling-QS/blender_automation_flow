...
from ...runtime_state.cache import (
    _auto_flow_bake_action_frame_range,
    _auto_flow_bake_action_has_cached_data,
    _auto_flow_bake_action_name_from_task_ref,
)


class RuntimeAutoFlowBakeExecutionMixin:
    def _import_background_auto_flow_bake_result(self, task_handle, payload, blend_copy_path):
        ...
        if not result_blend_path or not os.path.exists(result_blend_path):
            raise FlowExecutionError("AF_E005", "Background Auto Flow Bake result blend is missing", str(task_handle.get("node_name", "") or ""))

        action_name = str(report.get("action_name", "") or "")
        bake_asset_id = str(report.get("bake_asset_id", "") or "")
        if not action_name:
            action_name = _auto_flow_bake_action_name_from_task_ref(report)
        ...
        with bpy.data.libraries.load(result_blend_path, link=False) as (data_from, data_to):
            ...
            if action_name not in available_actions:
                raise FlowExecutionError("AF_E005", f"Background Auto Flow Bake action '{action_name}' was not found in result blend", str(task_handle.get("node_name", "") or ""))
        ...
        if not imported_actions:
            raise FlowExecutionError("AF_E005", f"Failed to import background Auto Flow Bake action '{action_name}'", str(task_handle.get("node_name", "") or ""))
        imported_action = imported_actions[0]

        existing_action = self._find_auto_flow_bake_action(bake_asset_id, action_name)
        ...
        self._tag_auto_flow_bake_action(imported_action, report)
        rebound_items = []
        ...
        for obj_item in object_items:
            ...
            try:
                ...
                if slot is None:
                    slot = self._ensure_auto_flow_bake_action_slot(imported_action, obj)
                try:
                ...
                continue
            rebound_items.append(self._auto_flow_bake_record_obj_item(obj))
    ...
    def _run_auto_flow_bake_once(self, start_tree, start_node_name, task_ref, frame, owner_node_name, bake_tracking=None):
        ...
        if isinstance(bake_tracking, dict):
            auto_flow_bake_context.update(bake_tracking)
        child_runner.auto_flow_bake_context = auto_flow_bake_context
        child_runner.start()
        unsupported_types = {"AFNodeTaskStep", "AFNodeRunTaskPlan", "AFNodeRunBackgroundTaskPlan", "AFNodeWaitForTask", "AFNodeReloadAfterTask"}
        for flow_node in list(getattr(child_runner, "nodes_in_order", []) or []):
            if str(getattr(flow_node, "bl_idname", "") or "") in unsupported_types:
                raise FlowExecutionError("AF_E005", f"Auto Flow Bake does not support '{flow_node.bl_label}'", owner_node_name)
        guard = 0
        while True:
            guard += 1
            if guard > 4096:
                raise FlowExecutionError("AF_E005", "Auto Flow Bake exceeded the maximum frame-step budget", owner_node_name)
            finished = child_runner.tick(max_immediate_steps=None)
            if finished:
                if str(getattr(child_runner, "status", "") or "") != STATUS_SUCCESS:
                    raise FlowExecutionError("AF_E005", "Auto Flow Bake child flow did not finish successfully", owner_node_name)
                return child_runner
            if child_runner.current_wait is not None or str(getattr(child_runner, "status", "") or "") in {STATUS_WAITING, STATUS_RELOADING}:
                raise FlowExecutionError("AF_E005", "Auto Flow Bake target flow cannot wait, reload, or run asynchronous tasks", owner_node_name)

    def _invoke_auto_flow_bake_task(self, node, task_ref):
        ...
        existing_action = self._find_auto_flow_bake_action(
            str(task_ref.get("bake_asset_id", "") or ""),
            str(task_ref.get("action_name", "") or ""),
        )
        if not bool(task_ref.get("free_before_bake", False)) and _auto_flow_bake_action_has_cached_data(existing_action):
            return {
                ...
                "skipped": True,
                "cache_frame_range": _auto_flow_bake_action_frame_range(existing_action),
            }

        previous_frame = int(getattr(self.scene, "frame_current", frame_start))
        frame_count = 0
        shared_action = self._ensure_auto_flow_bake_action(task_ref, clear_existing=True)
        bake_tracking = {
        ...
        try:
            for frame in range(frame_start, frame_end + 1):
                ...
                self._run_auto_flow_bake_once(start_tree, start_node_name, task_ref, frame, node.name, bake_tracking=bake_tracking)
                frame_count += 1
        finally:
...
__all__ = ["RuntimeAutoFlowBakeExecutionMixin"]
