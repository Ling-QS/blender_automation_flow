...
class RuntimeTaskPlanMixin:
        ...
        for step_ref in task_plan.get("step_refs", []):
            ...
            try:
                ...
                if invalid_issue is not None:
                    ...
                    continue
                if str(task_ref.get("task_kind", TASK_KIND_GEOMETRY)) != TASK_KIND_GEOMETRY:
                    continue
                try:
                    self._validate_background_geometry_task_support(task_ref, step_node.name)
                except FlowExecutionError as exc:
    ...
    def _import_background_task_plan_auto_flow_bake_results(self, state, status_payload):
        if not isinstance(state, dict) or not isinstance(status_payload, dict):
            return []
        handle = state.get("handle") or {}
        plan_report = dict(handle.get("report") or {})
        payload_report = dict(status_payload.get("report", {}) or {})
        auto_flow_reports = list(payload_report.get("auto_flow_bake_reports", []) or [])
        if not auto_flow_reports:
            return []

        blend_copy_path = str(state.get("blend_copy_path", "") or "")
        imported_reports = []
        for item in auto_flow_reports:
            if not isinstance(item, dict):
                continue
            imported_report = self._import_background_auto_flow_bake_result(
                handle,
        ...
        if imported_reports:
            plan_report["auto_flow_bake_reports"] = copy.deepcopy(imported_reports)
            plan_report["imported_auto_flow_bake_count"] = len(imported_reports)
        ...
        for step_ref in list(task_plan.get("step_refs", []) or []):
            ...
            try:
                ...
                if str(task_ref.get("task_kind", "") or "") != TASK_KIND_GEOMETRY:
                    continue
                if not bool(task_ref.get("apply_settings_on_run", False)):
                    continue
    ...
    def _start_background_task_plan(self, node, task_plan):
        ...
        if self._is_task_simulation_mode():
            ...
            return handle
        prepared_geometry_bake_settings = self._prepare_background_task_plan_geometry_bake_settings(node, task_plan)
    ...
    def _tick_background_task_plan(self, state):
        ...
        try:
            imported_reports = self._import_background_task_plan_auto_flow_bake_results(state, status_payload or {})
        except FlowExecutionError as exc:
        ...
        state["current_wait_type"] = ""
        if imported_reports:
            state["imported_auto_flow_bake_count"] = len(imported_reports)
        self._push_background_task_plan_outputs(state)
