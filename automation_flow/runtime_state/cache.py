...
def _auto_flow_bake_action_name_from_task_ref(task_ref):
    bake_asset_id = str(task_ref.get("bake_asset_id", "") or "").strip()
    if bake_asset_id:
        return f"{AUTO_FLOW_BAKE_ACTION_NAME_PREFIX}{bake_asset_id[:12]}"
    ...
    return f"{AUTO_FLOW_BAKE_ACTION_NAME_PREFIX}{fallback}"
...
def _auto_flow_bake_action_identifiers_from_node(node):
    bake_asset_id = str(getattr(node, "bake_asset_id", "") or "").strip()
    action_name = _auto_flow_bake_action_name_from_task_ref(
        {
            "bake_asset_id": bake_asset_id,
            "source_tree_name": getattr(getattr(node, "id_data", None), "name", ""),
            "source_node": getattr(node, "name", ""),
        }
    )
    return bake_asset_id, action_name


def _find_auto_flow_bake_action_by_identifier(bake_asset_id="", action_name=""):
    bake_asset_id = str(bake_asset_id or "").strip()
    action_name = str(action_name or "").strip()
    for action in bpy.data.actions:
        if bake_asset_id and str(action.get(AUTO_FLOW_BAKE_ACTION_PROP_BAKE_ASSET_ID, "") or "").strip() == bake_asset_id:
            return action
    if action_name:
...
def _find_auto_flow_bake_action_for_node(node):
    bake_asset_id, action_name = _auto_flow_bake_action_identifiers_from_node(node)
    return _find_auto_flow_bake_action_by_identifier(bake_asset_id, action_name)


def _auto_flow_bake_action_has_cached_data(action):
    if action is None:
        return False
    try:
...
def _auto_flow_bake_action_frame_range(action):
    if action is None or not _auto_flow_bake_action_has_cached_data(action):
        return None
    tagged_start = action.get(AUTO_FLOW_BAKE_ACTION_PROP_FRAME_START, None)
    tagged_end = action.get(AUTO_FLOW_BAKE_ACTION_PROP_FRAME_END, None)
    try:
...
def _auto_flow_bake_cache_status_from_node(node):
    if node is None or getattr(node, "bl_idname", "") != "AFNodeAutoFlowBakeTarget":
        return None
    ...
    return {
        "has_cache": True,
        "frame_range": _auto_flow_bake_action_frame_range(action),
        "source": "ACTION",
        "action_name": str(getattr(action, "name", "") or ""),
    }


def _auto_flow_bake_slot_display_name(obj):
    try:
