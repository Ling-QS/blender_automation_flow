...
class AF_OT_RemoveLastPropertyAssignmentInput(bpy.types.Operator):
    ...
    bl_label = "Remove Last Assignment Input"
    bl_description = "Remove the last real Property Assignment input from this Create Property Package node"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        if node is None or getattr(node, "bl_idname", "") != "AFNodeCreatePropertyPackage":
            self.report({"ERROR"}, "Create Property Package node not found")
            return {"CANCELLED"}
        ...
        if not removed:
            self.report({"INFO"}, "Create Property Package must keep at least one Property Assignment input")
            return {"CANCELLED"}
...
class AF_OT_AddPropertyAssignmentInput(bpy.types.Operator):
    ...
    bl_label = "Add Assignment Input"
    bl_description = "Add a new Property Assignment input to this Create Property Package node"
    bl_options = {"REGISTER", "UNDO"}
