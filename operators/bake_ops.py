...
from .bake_helpers import (
    ...
    _resolve_property_package_bake_target_node,
...
class AF_OT_FreeGNBakeCache(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            task_ref = _build_geometry_bake_task_ref_from_node(node)
            release_task_ref, force_release_settings = _build_geometry_bake_release_task_ref(task_ref)
            object_ref = task_ref["object_ref"]
            temporary_override = None
            original_state = None
            if force_release_settings:
                temporary_override, original_state = _apply_temporary_geometry_bake_settings(release_task_ref)
            else:
                temporary_override, original_state = _apply_current_geometry_bake_temporary_override(task_ref)
            override = {
                "object": object_ref,
            ...
            try:
                executed, last_tokens = _run_geometry_bake_free_operators(override, payload)
                if "FINISHED" in last_tokens:
                    _clear_geometry_bake_recorded_cache_state(task_ref)
                    _tag_flow_node_editor_redraw(getattr(getattr(node, "id_data", None), "name", None))
...
class AF_OT_FreePropertyPackageBakeCache(bpy.types.Operator):
    bl_idname = "af.free_property_package_bake_cache"
    bl_label = "Free Property Package Bake Cache"
    bl_description = "Delete recorded cache from the target Property Package Bake node"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        del context
        _node_tree, node = _resolve_property_package_bake_target_node(self.node_tree_name, self.node_name)
        if node is None:
            self.report({"ERROR"}, "Property Package Bake Target node not found")
            return {"CANCELLED"}

        try:
            action = _find_property_package_bake_action_for_node(node)
            if not _property_package_bake_action_has_cached_data(action):
                self.report({"INFO"}, "No Property Package Bake cache to free")
                return {"CANCELLED"}
            runner = FlowRunner(getattr(node, "id_data", None), getattr(bpy.context, "scene", None) or bpy.context.scene)
            runner._clear_property_package_bake_action_contents(action)
            _tag_flow_node_editor_redraw(getattr(getattr(node, "id_data", None), "name", None))
            self.report({"INFO"}, "Freed Property Package Bake cache")
            return {"FINISHED"}
        except Exception as exc:
            self.report({"ERROR"}, f"Failed to free Property Package Bake cache: {exc}")
            return {"CANCELLED"}
    ...
    def execute(self, context):
        ...
        try:
            ...
            if str(task_ref["physics_type"]) in {"CLOTH", "SOFT_BODY"}:
                ...
                if hasattr(modifier.point_cache, "use_disk_cache"):
                    modifier.point_cache.use_disk_cache = bool(task_ref.get("disk_cache", False))
                ...
                for surface in canvas_settings.canvas_surfaces:
                    ...
                    if point_cache is not None and hasattr(point_cache, "use_disk_cache"):
                        point_cache.use_disk_cache = bool(task_ref.get("disk_cache", False))
            else:
                self.report({"ERROR"}, "Supported physics bake modifier not found")
                return {"CANCELLED"}
        except Exception as exc:
...
BAKE_OPERATOR_CLASSES = (
    ...
    AF_OT_FreePropertyPackageBakeCache,
