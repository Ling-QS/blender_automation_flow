...
class RuntimeTaskRefDataMixin:
    def _group_node_has_task_plan_output(self, node):
        for output_socket in getattr(node, "outputs", []):
            if "task_plan" in set(self._socket_output_keys(output_socket)):
                return True
        return False
    ...
    def _evaluate_task_ref_data_node(self, node, node_type):
        ...
        if node_type == "AFNodeGroup":
            task_plan = None
            if self._group_node_has_task_plan_output(node):
            ...
            if isinstance(report_value, dict) and task_plan is not None:
            ...
            elif report_value is None and task_plan is not None:
                self._set_output(node, "report", {"step_count": int(task_plan["step_count"])})
            return True
        ...
        if node_type == "AFNodePropertyPackageBakeTarget":
            try:
                task_ref = self._build_property_package_bake_task_ref(node)
            except FlowExecutionError as exc:
                task_ref = self._make_invalid_task_ref_payload(node, exc)
            except Exception as exc:
                task_ref = self._make_invalid_task_ref_payload(node, exc)
        ...
        if node_type == "AFNodeRecordPropertyPackage":
            try:
                task_ref = self._build_record_property_package_task_ref(node)
            except FlowExecutionError as exc:
                task_ref = self._make_invalid_task_ref_payload(node, exc)
            except Exception as exc:
                task_ref = self._make_invalid_task_ref_payload(node, exc)
            ...
            report.update(
                {
                    "task_uid": str(task_ref.get("task_uid", "") or ""),
                    "record_tree_name": str(task_ref.get("record_tree_name", "") or ""),
                    "record_node_name": str(task_ref.get("record_node_name", "") or ""),
                    "start_node_name": str(task_ref.get("start_node_name", "") or ""),
                    "object_count": int(len(list(task_ref.get("predicted_object_items", []) or []))),
