...
from ...runtime_math.values import (
    ...
    _rotation_mode_values_to_payload,
)
...
class RuntimePropertyDataMixin:
    ...
    def _rotation_quaternions_match(self, lhs_rotation, rhs_rotation, epsilon=1e-6):
        try:
            lhs_quaternion = _payload_to_quaternion(lhs_rotation)
            rhs_quaternion = _payload_to_quaternion(rhs_rotation)
            dot = abs(float(lhs_quaternion.dot(rhs_quaternion)))
        except Exception:
            return False
        return abs(1.0 - dot) <= max(1e-8, float(epsilon))

    def _resolved_rotation_payload_mode_and_values(self, obj, rotation_payload):
        fallback_mode = str(getattr(obj, "rotation_mode", "XYZ") or "XYZ")
        quaternion_value = _payload_to_quaternion(rotation_payload)
        if fallback_mode == "QUATERNION":
        ...
        if fallback_mode == "AXIS_ANGLE":
            angle = float(quaternion_value.angle)
            axis = quaternion_value.axis if angle > 1e-8 else Vector((0.0, 0.0, 1.0))
            return fallback_mode, [
                angle,
                float(axis.x),
                float(axis.y),
        ...
        euler_mode = fallback_mode if fallback_mode in {"XYZ", "XZY", "YXZ", "YZX", "ZXY", "ZYX"} else "XYZ"
        euler_value = quaternion_value.to_euler(euler_mode)
        return euler_mode, [
            float(euler_value.x),
            float(euler_value.y),
            float(euler_value.z),
    ...
    def _rotation_payload_matches_object(self, obj, rotation_payload):
        if rotation_payload is None:
            return False
        return self._rotation_quaternions_match(
            self._capture_object_rotation_value(obj),
            rotation_payload,
        )
    ...
    def _capture_object_rotation_value(self, obj, rotation_mode=None):
        ...
        if resolved_mode == "QUATERNION":
            rotation_values = [float(value) for value in getattr(obj, "rotation_quaternion", (1.0, 0.0, 0.0, 0.0))]
        elif resolved_mode == "AXIS_ANGLE":
            rotation_values = [float(value) for value in getattr(obj, "rotation_axis_angle", (0.0, 0.0, 0.0, 1.0))]
        else:
            rotation_values = [float(value) for value in getattr(obj, "rotation_euler", (0.0, 0.0, 0.0))]
        return _rotation_mode_values_to_payload(resolved_mode, rotation_values)

    def _apply_object_rotation_value(self, obj, rotation_payload, dry_run=False):
        if rotation_payload is None:
            return
        rotation_mode, rotation_values = self._resolved_rotation_payload_mode_and_values(obj, rotation_payload)
        if not dry_run:
    ...
    def _object_transform_property_assignment_from_node(self, node):
        ...
        if bool(properties["rotation"]) and sources["rotation"] == PROPERTY_SOURCE_VALUE:
            ...
            if linked_rotation is not None:
                ...
                values["rotation"] = _rotation_mode_values_to_payload(rotation_mode, rotation_value)
