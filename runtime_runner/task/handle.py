...
class RuntimeTaskHandleMixin:
    ...
    def _rehydrate_property_package_bake_predicted_items(self, task_ref, scene):
        ...
        try:
            ...
            record_tree_name = str(task_ref.get("record_tree_name", "") or task_ref.get("source_tree_name", "") or "")
            record_node_name = str(task_ref.get("record_node_name", "") or task_ref.get("source_node", "") or "").strip()
            if not record_tree_name or not record_node_name:
                return []
            ...
            if record_node is None or getattr(record_node, "bl_idname", "") != "AFNodeRecordPropertyPackage":
                return []
            previous_group_path = list(getattr(self, "current_group_path", []) or [])
            try:
                self.current_group_path = list(task_ref.get("record_group_path", []) or [])
                source_ref = self._build_record_property_package_task_ref(record_node)
            finally:
                self.current_group_path = previous_group_path
            targets = {"predicted_object_items": list(source_ref.get("predicted_object_items", []) or [])}
        except Exception:
            return []
