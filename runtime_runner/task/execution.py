...
from ...runtime_task_target import (
    ...
    _register_returned_physics_bake_all_disk_cache as _register_returned_physics_bake_all_disk_cache_impl,
    _register_returned_physics_disk_cache as _register_returned_physics_disk_cache_impl,
...
class RuntimeTaskExecutionMixin:
    def _is_external_process_execution(self):
        return bool(getattr(self, "_external_process_execution", False))

    def _prepare_task_ref_for_runtime_execution(self, task_ref):
        if not isinstance(task_ref, dict):
            return task_ref
        task_kind = str(task_ref.get("task_kind", TASK_KIND_GEOMETRY) or TASK_KIND_GEOMETRY)
        if not self._is_external_process_execution():
            return task_ref
        if task_kind == TASK_KIND_PHYSICS:
            prepared = dict(task_ref)
            prepared["_force_disk_cache"] = True
            return prepared
        if task_kind == TASK_KIND_PHYSICS_BAKE_ALL:
            prepared = dict(task_ref)
            prepared["_force_disk_cache"] = True
            prepared["tasks"] = [dict(item) if isinstance(item, dict) else item for item in list(task_ref.get("tasks", []) or [])]
            for item in prepared["tasks"]:
                if isinstance(item, dict):
                    item["_force_disk_cache"] = True
            return prepared
        return task_ref
    ...
    def _build_geometry_bake_release_task_ref(self, task_ref):
        return _build_geometry_bake_release_task_ref_impl(
            task_ref,
            get_geometry_bake_last_bake_state=self._get_geometry_bake_last_bake_state,
    ...
    def _apply_geometry_bake_runtime_disk_directory(self, bake_entry, task_ref, *, resolve_default_directory=None):
        return _apply_geometry_bake_runtime_disk_directory_impl(
            bake_entry,
            task_ref,
            resolve_default_directory=resolve_default_directory or (
                lambda current_task_ref: self._geometry_bake_default_disk_cache_root_dir_relpath(
                    current_task_ref
                )
            ),
        )

    def _geometry_bake_default_disk_cache_root_dir_relpath(self, task_ref):
        object_ref = self._require_payload_object_ref(task_ref, str(task_ref.get("source_node", "") or "geometry bake"))
        modifier = object_ref.modifiers.get(task_ref["modifier_name"])
        if modifier is None:
            raise FlowExecutionError(
                "AF_E017",
                f"Modifier '{task_ref['modifier_name']}' missing while resolving default GN disk cache directory",
                str(task_ref.get("source_node", "") or "geometry bake"),
            )
        from ...runtime_task_target import _geometry_bake_default_disk_cache_root_dir_relpath as _impl

        return _impl(task_ref, modifier, bpy_module=bpy)
    ...
    def _read_geometry_bake_last_bake_state(self, node):
    ...
    def _write_geometry_bake_last_bake_state(self, task_ref, frame_range=None):
    ...
    def _clear_geometry_bake_last_bake_state(self, task_ref):
    ...
    def _get_geometry_bake_last_bake_state(self, task_ref):
        return _get_geometry_bake_last_bake_state_impl(
            task_ref,
            resolve_geometry_task_source_node=self._resolve_geometry_task_source_node,
            read_geometry_bake_last_bake_state=self._read_geometry_bake_last_bake_state,
        )
    ...
    def _apply_temporary_geometry_bake_settings(self, task_ref):
        return _apply_temporary_geometry_bake_settings_impl(
            ...
            apply_geometry_bake_runtime_disk_directory=self._apply_geometry_bake_runtime_disk_directory,
            resolve_default_directory=self._geometry_bake_default_disk_cache_root_dir_relpath,
        )

    def _apply_temporary_geometry_bake_runtime_directory(self, task_ref):
        return _apply_temporary_geometry_bake_settings_impl(
            task_ref,
    ...
    def _apply_current_geometry_bake_temporary_override(self, task_ref):
        return _apply_current_geometry_bake_temporary_override_impl(
            ...
            apply_temporary_geometry_bake_runtime_directory=self._apply_temporary_geometry_bake_runtime_directory,
        )

    def _clear_geometry_bake_recorded_cache_state(self, task_ref):
        return _clear_geometry_bake_recorded_cache_state_impl(
            ...
            clear_geometry_bake_last_bake_state=self._clear_geometry_bake_last_bake_state,
        )

    def _normalize_geometry_bake_modifier_directory_for_task(self, task_ref, bake_entry=None):
    ...
    def _register_returned_physics_disk_cache(self, task_ref, scene=None, ui_context=None):
        return _register_returned_physics_disk_cache_impl(
            task_ref,
            scene if scene is not None else getattr(bpy.context, "scene", None),
    ...
    def _register_returned_physics_bake_all_disk_cache(self, task_ref, scene=None, ui_context=None):
        return _register_returned_physics_bake_all_disk_cache_impl(
            task_ref,
            scene if scene is not None else getattr(bpy.context, "scene", None),
            ui_context=ui_context,
            bpy_module=bpy,
    ...
    def _start_geometry_nodes_bake_task(self, task_ref, scene, ui_context=None):
        return _start_geometry_nodes_bake_task_impl(
            ...
            apply_temporary_geometry_bake_settings=self._apply_temporary_geometry_bake_settings,
            apply_current_geometry_bake_temporary_override=self._apply_current_geometry_bake_temporary_override,
            build_geometry_bake_release_task_ref=self._build_geometry_bake_release_task_ref,
            normalize_geometry_bake_modifier_directory_for_task=self._normalize_geometry_bake_modifier_directory_for_task,
            clear_geometry_bake_recorded_cache_state=self._clear_geometry_bake_recorded_cache_state,
            call_operator_with_override=self._call_operator_with_override,
    ...
    def _invoke_geometry_nodes_bake_task(self, task_ref, scene):
        return _invoke_geometry_nodes_bake_task_impl(
            ...
            start_named_operator=self._start_named_operator,
            refresh_geometry_bake_cache_state_after_completion=self._refresh_geometry_bake_cache_state_after_completion,
            schedule_geometry_bake_cache_refresh=self._schedule_geometry_bake_cache_refresh,
            resolve_geometry_task_effective_bake_target=self._resolve_geometry_task_effective_bake_target,
            geometry_bake_entry_has_cached_data=self._geometry_bake_entry_has_cached_data,
            geometry_bake_disk_cache_exists=self._geometry_bake_disk_cache_exists,
            clear_geometry_bake_tracked_packed_cache_state=self._clear_geometry_bake_tracked_packed_cache_state,
            write_geometry_bake_tracked_packed_cache_state=self._write_geometry_bake_tracked_packed_cache_state,
            write_geometry_bake_last_bake_state=self._write_geometry_bake_last_bake_state,
    ...
    def _run_task_ref_now(self, node, task_ref, task_handle=None):
        ...
        task_ref = self._prepare_task_ref_for_runtime_execution(task_ref)
        ...
        if dry_run or flow_test:
            ...
            if task_kind == TASK_KIND_PHYSICS:
                ...
                property_package_bake_report = self._invoke_property_package_bake_task(node, task_ref)
                task_handle["report"] = dict(property_package_bake_report)
                result = {"SKIPPED"} if bool(property_package_bake_report.get("skipped", False)) else {"FINISHED"}
    ...
    def _start_task_ref_async(self, node, task_ref, object_list, assign_wait=True):
        ...
        task_kind = str(task_ref.get("task_kind", TASK_KIND_GEOMETRY))
        if task_kind in {TASK_KIND_RENDER, TASK_KIND_PROPERTY_PACKAGE_BAKE} or not self._supports_async_bake():
    ...
    def _poll_bake_wait_state(self, node, wait):
        ...
        if str(task_ref.get("task_kind", "")) == TASK_KIND_GEOMETRY:
            ...
            effective_target = self._resolve_geometry_task_effective_bake_target(task_ref)
            ...
            try:
                bake_entry = self._find_geometry_bake_entry_for_task(task_ref, "checking geometry bake cache after completion")
