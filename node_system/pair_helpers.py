...
def _start_node_active_updated(self, context):
    ...
    _invalidate_auto_follow_start_cache()
...
def _start_node_auto_order_updated(self, context):
    del context
    _invalidate_auto_follow_start_cache()


def _invalidate_auto_follow_start_cache():
    try:
        from ..operators import flow_run as flow_run_module
    except Exception:
        return
    mark_dirty = getattr(flow_run_module, "_mark_auto_follow_start_cache_dirty", None)
