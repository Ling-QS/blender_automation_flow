...
def _restore_waiting_bake_cleanup(
    ...
    *,
    ...
    if kind == task_kind_physics:
        ...
        keep_overridden_disk_cache = bake_completed and bool(cleanup.get("keep_overridden_disk_cache_on_success", False))
        ...
        if modifier is not None and not keep_overridden_disk_cache:
            original_disk_cache = cleanup.get("original_disk_cache")
            if original_disk_cache is not None and hasattr(modifier, "point_cache") and hasattr(modifier.point_cache, "use_disk_cache"):
                modifier.point_cache.use_disk_cache = bool(original_disk_cache)
            original_surface_disk_cache = cleanup.get("original_surface_disk_cache")
            if original_surface_disk_cache:
                for surface, disk_cache_enabled in original_surface_disk_cache:
                    point_cache = getattr(surface, "point_cache", None)
                    if point_cache is None or not hasattr(point_cache, "use_disk_cache"):
                        continue
                    point_cache.use_disk_cache = bool(disk_cache_enabled)
        restore_scene_frame_state(cleanup.get("scene"), cleanup.get("scene_state"))
        return
    if kind == task_kind_physics_bake_all:
        ...
        if not (bake_completed and bool(cleanup.get("keep_overridden_disk_cache_on_success", False))):
            for modifier, disk_cache_enabled in cleanup.get("original_disk_cache", []):
...
def _refresh_geometry_bake_cache_state_after_completion(
    ...
    *,
    ...
    if task_target != "DISK" and not geometry_bake_entry_has_cached_data(bake_entry):
        # Keep the inline refresh short so flow completion does not stall the UI;
        # the deferred timer path can continue probing when Blender reports cache late.
    ...
    def _timer():
        ...
        try:
            ...
            has_cache = bool(
                ...
                or geometry_bake_disk_cache_exists(task_ref_copy, bake_entry=bake_entry)
            )
        except Exception:
            has_cache = False
        ...
        if has_cache:
            try:
                refresh_geometry_bake_cache_state_after_completion(task_ref_copy, scene_ref)
            except Exception:
                pass
            if tag_all_node_editor_redraw is not None:
                tag_all_node_editor_redraw()
