...
def build_display_helpers(
    *,
    ...
    def _draw_compact_property_source(row, node, source_attr, enabled=True):
        ...
        source_row.scale_x = 0.56
        try:
            source_row.ui_units_x = 2.15
        except Exception:
            pass
        value_row = source_row.row(align=True)
        current_row = source_row.row(align=True)
        try:
            value_row.ui_units_x = 1.0
            current_row.ui_units_x = 1.0
        except Exception:
            pass
        value_row.prop_enum(node, source_attr, PROPERTY_SOURCE_VALUE, text="V")
        current_row.prop_enum(node, source_attr, PROPERTY_SOURCE_CURRENT, text="C")
        return source_row
